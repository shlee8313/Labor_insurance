...
const CalendarWorkTime = ({
  ...
  const renderDateCell = (index) => {
    ...
    // 🔥 데이터가 입력되었는지 확인 (세분화된 시간 중 하나라도 입력되었는지)
    const hasTimeData =
    ...
    // 🔥 체크박스 상태 확인 - 값이 있고 0보다 큰 경우에만 체크된 것으로 간주
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-1">
          <input
            ...
            disabled={isPaid}
            title={isPaid ? "지급완료된 기록은 수정할 수 없습니다" : "일반 근무시간"}
          />
          <input
            type="number"
            step="0.5"
          ...
          <label className={`flex items-center gap-1 ${isPaid ? "opacity-50" : ""}`}>
            <input
              ...
              onChange={(e) => {
                if (!isPaid) {
                  if (e.target.checked) {
                    handleChange(dayIndex, "overtime_hours", "1"); // 체크시 기본값 1 설정
                  } else {
                    handleChange(dayIndex, "overtime_hours", ""); // 체크 해제시 빈값
                  }
                }
              }}
              disabled={isPaid}
          ...
          <label className={`flex items-center gap-1 ${isPaid ? "opacity-50" : ""}`}>
            <input
              ...
              onChange={(e) => {
                if (!isPaid) {
                  if (e.target.checked) {
                    handleChange(dayIndex, "holiday_hours", "1"); // 체크시 기본값 1 설정
                  } else {
                    handleChange(dayIndex, "holiday_hours", ""); // 체크 해제시 빈값
                  }
                }
              }}
              disabled={isPaid}
          ...
          <label className={`flex items-center gap-1 ${isPaid ? "opacity-50" : ""}`}>
            <input
              ...
              onChange={(e) => {
                if (!isPaid) {
                  if (e.target.checked) {
                    handleChange(dayIndex, "night_hours", "1"); // 체크시 기본값 1 설정
                  } else {
                    handleChange(dayIndex, "night_hours", ""); // 체크 해제시 빈값
                  }
                }
              }}
              disabled={isPaid}
          ...
          <div className="grid grid-cols-3 gap-1">
            {hasOvertimeHours ? (
              <input
                ...
                disabled={isPaid}
                title={isPaid ? "지급완료된 기록은 수정할 수 없습니다" : "연장 근무시간"}
              />
            ) : (
              <div></div>
            )}
            ...
            {hasHolidayHours ? (
              <input
                ...
                disabled={isPaid}
                title={isPaid ? "지급완료된 기록은 수정할 수 없습니다" : "휴일 근무시간"}
              />
            ) : (
              <div></div>
            )}
            ...
            {hasNightHours ? (
              <input
                ...
                disabled={isPaid}
                title={isPaid ? "지급완료된 기록은 수정할 수 없습니다" : "야간 근무시간"}
              />
            ) : (
              <div></div>
            )}
        ...
        <input
          ...
          disabled={isPaid}
          title={isPaid ? "지급완료된 기록은 수정할 수 없습니다" : "기본 일당"}
        />
        ...
        <>
          ...
          <div className="mt-4 p-3 bg-blue-50 rounded-md border border-blue-200">
            ...
            <ul className="space-y-1 text-sm text-blue-700">
              <li>
                • <strong>일반시간</strong>: 기본 근무시간을 입력하세요
              </li>
              <li>
                • <strong>연장/야간/휴일</strong>: 해당 근무가 있는 경우 체크박스를 클릭하여
                활성화한 후 시간을 입력하세요
              </li>
              <li>
                • <strong>일당</strong>: 해당일의 기본 일당을 입력하세요
              </li>
              <li>
                • <strong>지급완료</strong> 표시된 날짜는 수정할 수 없습니다
              </li>
            </ul>
          </div>
        </>
