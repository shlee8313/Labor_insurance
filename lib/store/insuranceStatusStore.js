...
/**
 * 4대보험 가입 자격 상태 관리 전담 스토어
 * 역할: 보험 상태 계산, 수동 설정 관리, UI 유틸리티 제공
 */
const useInsuranceStatusStore = create((set, get) => ({
  ...
  _fetchWorkHistory: async (workerId, siteId, yearMonth) => {
    ...
    console.log(`[DEBUG] 근무 이력 조회 - 이전 달: ${prevYearMonth}, 선택 달: ${yearMonth}`);
    ...
    if (currRecords && currRecords.length > 0) {
      ...
      console.log(`[DEBUG] 이번달 마지막 근무일: ${lastWorkDateThisMonth}`);
    }

    console.log(`[DEBUG] 조회 결과:`, {
    ...
    if (manualSetting) {
      ...
      insuranceTypes.forEach((type) => {
        ...
        if (manualSetting[statusField]?.startsWith("manual_")) {
          ...
          finalStatus[statusKey].reason = "수동 설정";
  ...
  getEffectiveStatus: (workerId, siteId, yearMonth, insuranceType) => {
    ...
    return {
      ...
      reason: status[statusKey].reason || "정보 없음",
  ...
  recalculateAllInsuranceStatus: async (siteId, yearMonth, workersList) => {
    ...
    try {
      ...
      console.error("보험 상태 일괄 재계산 오류:", error);
      set({ isLoading: false, error: error.message });
    }
  },
